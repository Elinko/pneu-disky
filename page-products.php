 ...
 */
	...
	if(!empty(htmlspecialchars($_GET["cat"]))) {
	  ...
	  $catParent2 = get_category($catParent->parent);
		...
		if($catParent2->errors['invalid_term'][0]!='Prázdny termín.') {
		...
		echo "<pre>";
		print_r($catParent2->errors['invalid_term'][0]);
		echo "</pre>";
		...
		<div id="fh5co-blog-section">
			<div class="container">
				<div class="heading-section">
					...
					<h3 class="text-center"><?php if($catParent) {echo 'Lacné ' .($catParent) .' značky '. ucfirst($catName->name ); } else { echo 'Lacné '. ($catName->name)  ;}   ?></h3>
				</div>
				<div class="row">
